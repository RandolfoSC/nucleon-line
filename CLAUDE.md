...
- **v1.1 — Hero em vídeo.** Primeira exceção à regra "sem vídeos" da v1.0, só na Dobra 1 e só em desktop (≥768px) — decisão da Direção Criativa, documentada como decisão travada 8 (§2). O resto da v1.0 continua valendo.
...
8. **Hero em vídeo (D1 · v1.1).** Primeira exceção à regra "sem vídeos" da v1.0 (§1) — decisão da Direção Criativa, não uma reabertura por conta própria da Engenharia. Vídeo real (`assets/video/hero.mp4`, 16:9) substitui a foto estática da Dobra 1, mas **só em telas ≥768px**: testado nos dois lados do breakpoint, e um vídeo 16:9 forçado a cobrir uma tela retrato estreita corta tanto da lateral pra cobrir a altura que o sujeito (a jovem) desaparece do enquadramento por completo — não sobra recorte, em nenhum `object-position`, que a mantenha visível. Abaixo de 768px, o fallback é uma foto estática (`assets/img/d1-hero-mobile.webp`) extraída do próprio vídeo em 4.5s (o momento em que ela sorri pro celular) e pré-recortada centrada nela via `ffmpeg` — resolve o mesmo problema que a margem de segurança horizontal do `PROMPTS_FREEPIK.md` resolve pras fotos geradas, só que aplicada a posteriori, porque a fonte é um frame de vídeo, não uma geração já composta com essa margem.
   **Execução travada — carregamento condicional:** o `<video>` nasce em `index.html` SEM `src` (só `data-src`) e SEM `autoplay`. `main.js` → `initHeroVideoResponsivo()` só atribui o `src` (e chama `.play()`) quando `matchMedia("(min-width: 768px)")` bate, e remove o `src` de volta (+ `.load()`) se a janela encolher pra mobile. Um `<video autoplay>` começa a baixar dados assim que o navegador processa a tag, MESMO com `display:none` no CSS — só esconder visualmente não evita o gasto de dados de quem só vai ver o fallback. `display:none`/`block` a 767px/768px (puro CSS, mesmo corte do JS) decide qual mídia *aparece*; `initHeroVideoResponsivo()` decide qual mídia *é baixada*.
   **Execução travada — corte sem distorcer:** vídeo e foto mobile compartilham a classe `.hero__midia` (`width:130%; height:130%` — os DOIS eixos, nunca só a altura como o `.dobra__foto` genérico das fotos com overscan real). Escalar só a altura distorce a proporção da caixa que o `object-fit:cover` usa pra calcular o corte, forçando um corte horizontal bem maior que o necessário — medido em ~34% da largura do vídeo cortada contra ~1% escalando os dois eixos igualmente, no mesmo container de teste. Foi esse corte extra, não o parallax em si, que empurrava o sujeito pra fora do quadro nos primeiros testes.
   **Execução travada — logo sincronizada:** `.hero__logo`, sobreposta ao vídeo (não ao fallback mobile), faz fade-in (~0.6s, `--transicao-tema`) quando `video.currentTime` atinge 4s (o sorriso). `initHeroLogoSync()` escuta `timeupdate` e aplica `classList.toggle("hero__logo--visivel", currentTime >= 4)` a cada frame — como o vídeo está em loop, o mesmo listener resolve o fade-out sozinho (ao reiniciar, `currentTime` volta abaixo de 4s, a classe some, a transição cuida do fade). Não aparece no mobile: sem vídeo tocando, não existe `timeupdate` pra disparar.
   **Guard de `prefers-reduced-motion`:** o parallax do Hero segue o MESMO guard das outras dobras (§6) — `construirParallax()` passou a percorrer TODOS os `.dobra__foto` de uma `.dobra__media` (não só o primeiro), porque agora a D1 tem dois candidatos (`.hero__video` e `.hero__foto-mobile`) na mesma `.dobra__media`; o que estiver com `display:none` simplesmente não é pintado. O autoplay do vídeo em si NÃO é desligado por movimento reduzido — é loop simples, sem deslocamento adicional, mesmo raciocínio que mantém o Arco de Luz ativo sob movimento reduzido (é conteúdo/cor, não parallax).
   **Nota de ferramenta:** `dev-server.js` (servidor local de desenvolvimento, não usado em produção — quem serve o site em produção é a Vercel) precisou ganhar suporte a `Range` requests (cabeçalho `Accept-Ranges`, resposta `206 Partial Content` fatiada via `fs.createReadStream`) — sem isso o `<video>` não consegue dar seek nem, em alguns navegadores, iniciar o autoplay corretamente; um `fs.readFile` simples devolvendo o arquivo inteiro de uma vez não é suficiente pra vídeo.
...
- **D1 (Hero · v1.1):** "Educação que abre portas para quem o mercado ainda não enxergou." + "O Instituto Nucleon-line leva capacitação profissional online e acessível a jovens, mulheres e populações vulneráveis de todo o Brasil — direto no celular, onde quer que você esteja." — CTAs: [Quero Estudar] [Quero Apoiar o Projeto]. Substitui a copy original da v1.0 ("O mercado não espera. Ele seleciona." — [Inicie Sua Trajetória] [Saiba Mais]), registrada aqui só por histórico.
...
- **Hero em vídeo (D1 · v1.1 — feito):** vídeo real em desktop (≥768px), foto estática (frame do próprio vídeo em 4.5s) em mobile — ver decisão travada 8 (§2) para a auditoria completa (por que não dá pra usar o vídeo no mobile, carregamento condicional via `initHeroVideoResponsivo()`, correção de corte via `.hero__midia`, logo sincronizada via `initHeroLogoSync()`, e o guard de `prefers-reduced-motion` reaproveitado do parallax).
